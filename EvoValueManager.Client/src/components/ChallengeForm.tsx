...
const defaultFormData: Omit<Challenge, 'id'> = {
    title: '',
    requiredBravery: null, requiredTrust: null, requiredPresence: null, requiredGrowth: null, requiredCare: null,
    gainableBravery: null, gainableTrust: null, gainablePresence: null, gainableGrowth: null, gainableCare: null
};
...
function ChallengeForm({initialData, onSubmit, onCancel, isSaving}: ChallengeFormProps) {
    ...
    // @ts-ignore
    const renderStatInput = (statType: 'required' | 'gainable', traitProperty: string, traitTitle: string) => {
        const name = `${statType}` + capitalize(`${traitProperty}`) as keyof Challenge;
    ...
    return (
        <form onSubmit={handleSubmit} className="challenge-form">
            ...
            <div className="form-group">
                <label htmlFor="title">Kihívás neve:</label>
                <input
                    className="form-control"
                    type="text"
                    id="title"
            ...
            <fieldset className="stats-fieldset">
                ...
                <div className="stats-grid">
                    {TRAITS.map(trait => renderStatInput('required', trait.property, trait.title))}
                </div>
            </fieldset>
            ...
            <fieldset className="stats-fieldset">
                ...
                <div className="stats-grid">
                    {TRAITS.map(trait => renderStatInput('gainable', trait.property, trait.title))}
                </div>
            </fieldset>
            ...
            <div className="form-actions">
                <button type="submit" disabled={isSaving}>
                    {isSaving ? 'Saving...' : (initialData ? 'Változtatások mentése' : 'Kihivás hozzáadása')}
                </button>
                {onCancel && (
