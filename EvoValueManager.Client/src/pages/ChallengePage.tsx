...
function ChallengePage() {
    ...
    const handleFormSubmit = async (challengeData: Omit<Challenge, 'id'> | Challenge) => {
        setIsSaving(true);
        setError(null);
        try {
            ...
            const message = err.response?.data?.message || err.response?.data || 'Failed to save challenge.';
            setError(message);
            console.error(err);
        } finally {
            setIsSaving(false);
