...
function CharacterPage() {
    ...
    const handleFormSubmit = async (characterData: Omit<Character, 'id'> | Character) => {
        setIsSaving(true);
        setError(null);
        try {
            if ('id' in characterData) {
                await api.updateCharacter(characterData.id, characterData);
                setCharacters(prev => prev.map(c => c.id === characterData.id ? characterData : c));
            ...
            const message = err.response?.data?.message || err.response?.data || 'Failed to save character.';
            setError(message);
            console.error(err);
        } finally {
            setIsSaving(false);
    ...
    return (
        <div className="page-container">
            <h1>Csapattagok</h1>
            {error && !isSaving && <p className="error-message">Error loading page: {error}</p>}
